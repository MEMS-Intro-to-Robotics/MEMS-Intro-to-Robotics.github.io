...
ssh -T git@github.com</code></pre>
        ...
        <li><strong>Pull the Kinova course image.</strong> This is a different image from Labs 2&ndash;4.
            <p><strong>Location:</strong> Host VM Terminal</p>
...
cd intro-to-robotics-fall-2026-lab-05-YOUR_GITHUB_USERNAME</code></pre>
    ...
    <ol start="6">
        ...
        <li><strong>Start the course container.</strong> The command names the container <code>lab05</code>, makes your VM&rsquo;s <code>~/workspaces</code> folder available inside it, and gives Gazebo and RViz access to the VM&rsquo;s GPU through <code>--gpus all</code>.
            <!-- TODO(instructor): --gpus all verified by setup script (vm_setup.sh installs nvidia-container-toolkit) but not yet run on a student VM with the Kinova image. -->
            <p><strong>Location:</strong> Host VM Terminal</p>
...
docker run --rm -it \
  ...
  ghcr.io/mems-intro-to-robotics/mems-robotics-toolkit:kinova-jazzy-latest</code></pre>
...
ros2 pkg create --build-type ament_python lab05_moveit \
  --dependencies rclpy geometry_msgs sensor_msgs std_msgs std_srvs \
  visualization_msgs tf2_ros moveit_msgs pymoveit2
...
touch lab05_moveit/lab05_moveit/scripts/__init__.py</code></pre>
        ...
        <li><strong>Copy the three starter files into the package.</strong> Leave the originals in <code>scaffolds/</code> as a clean copy to fall back on.
            <p><strong>Location:</strong> Host VM Terminal, from your repository root</p>
            <pre><code class="language-bash">cp scaffolds/motion_planner.py scaffolds/m5_detour.py scaffolds/pen.py \
  ros2_ws/src/lab05_moveit/lab05_moveit/scripts/</code></pre>
            <ul>
...
git push origin main</code></pre>
    ...
    <ul>
        ...
        <li>[ ] <code>ls ros2_ws/src/lab05_moveit/lab05_moveit/scripts</code> shows <code>__init__.py</code>, <code>m5_detour.py</code>, <code>motion_planner.py</code>, and <code>pen.py</code>.</li>
...
docker image inspect ghcr.io/mems-intro-to-robotics/mems-robotics-toolkit:kinova-jazzy-latest &gt; /dev/null</code></pre>
        <p><strong>Checkpoint:</strong> The working tree is clean on <code>main</code>, the remote is your private Lab 5 repository, the listing shows the four files in <code>scripts/</code>, and the image-inspection command exits without an error. Stop here and finish the pre-lab if any check fails.</p>
    </section>
    <section id="part2">
...
Error: According to the loaded plugin descriptions the class gz_ros2_control/GzSimSystem ...</code></pre>
        ...
        <p>Wait for <code>You can start planning now!</code> in this terminal. An RViz window opens showing the robot.</p>
        ...
        <p>In the RViz window, choose <strong>File &rarr; Open Config</strong> and open <code>lab05.rviz</code> from your repository (under <code>/root/workspaces/intro-to-robotics-fall-2026-lab-05-YOUR_GITHUB_USERNAME/</code>). When RViz asks whether to save the current configuration, choose not to.</p>
        ...
        <p><strong>Checkpoint:</strong> Terminal 3 prints <code>Pen ready: tracing end_effector_link in base_link. Pen is up</code>. The Gazebo arm and the RViz arm are in the same pose. In Terminal 4 (prepared), <code>ros2 control list_controllers</code> shows <code>joint_trajectory_controller</code>, <code>joint_state_broadcaster</code>, and <code>gen3_lite_2f_gripper_controller</code> as <code>active</code>.</p>
    ...
    <section id="part3">
        ...
        <p>In RViz, open the <strong>MotionPlanning</strong> panel&rsquo;s <strong>Joints</strong> tab and move the sliders to a folded, compact pose that keeps the gripper above the ground and clear of the robot&rsquo;s own base. The sliders show degrees; convert to radians. Replace the six values in <code>self.retract_joints</code> with yours.</p>
        ...
        <div style="background-color: #f8f9fa; border-left: 4px solid #005a9c; padding: 1em; margin-top: 1em; border-radius: 4px;">
            <pre style="margin: 0; font-family: monospace; font-size: 0.9em;"><code>[INFO] [motion_planner_node]: Retract reached. Measured joint positions: +0.000, -1.200, +1.399, -0.000, +1.100, +0.001</code></pre>
        </div>
    ...
    cartesian_fraction_threshold=0.9,
        ...
        <details>
            ...
            <p>The pen traces <code>end_effector_link</code>, which sits inside the gripper. A segment that points back toward the wrist can be hidden by the gripper model from some viewpoints. Orbit the RViz view until both segments are visible.</p>
        </details>
...
time.sleep(1.0)</code></pre>
    ...
    <section id="part7">
        ...
        <ul>
            ...
            <li>Remove the box at the end so the scene is clean for the next run.</li>
        ...
        <details>
            ...
            <p>Draw the no-obstacle path first and inspect it. If the box does not intersect that line, the original path remains collision free and the planner may return it unchanged. Place the box across the line so the direct path is blocked.</p>
        </details>
        <details>
...
<section id="analysis">
    ...
    <ul>
        <li>Change one of your milestone 2 segments to a length the arm cannot reach in a straight line (for example 0.40 m) and run it. What did the planner return, and what did MoveIt log?</li>
